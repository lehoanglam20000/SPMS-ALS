...
public class Cromosoma implements Comparable {
  ...
  public double optimizacionLocal (int nClases, int K, int clases[], double dMatrix[][], double umbral, double datos[][], double real[][], int nominal[][], boolean nulo[][], boolean distanceEu) {
	  ...
	  n = this.genesActivos();
	  visitas = new int[n]; //Index of selected instances the chromosome
	  ...
	  for (j=0; j<visitas.length; j++) { //Shuffle the indexes of selected instances in visitas
	  ...
	  while (i < n) {
	      ...
	      if (ev >= 0) {//cuerpo was modified in evaluacionParcial, need to update the visitas
	    	  n--;
	    	  i = 0;
	    	  visitas = new int[n];
	    	  try {
	    	  ...
	    	  for (j=0; j<visitas.length; j++) {//Shuffle the indexes of visitas
	    		  pos = Randomize.Randint (j, visitas.length-1);
	    		  tmp = visitas[j];
	    		  visitas[j] = visitas[pos];
	    		  visitas[pos] = tmp;
  ...
  public double evaluacionParcial (int nClases, int K, int clases[], int ref, double dMatrix[][], double umbral, double datos[][], double real[][], int nominal[][], boolean nulo[][], boolean distanceEu) {
	  ...
	  cuerpo[ref] = false; //drop the instance at ref
	  for (i=0; i<cuerpo.length; i++) {
	      evaluar = false;
	      for (j=0; j<K; j++) {
	    	  if (vecinos[i][j] == ref) { // vecinos = neighbor which is a pre-computed matrix of neighbours for each instance
	    		  evaluar = true;
	    		  //We check if we dont use the instance at position ref, what is the second closest instance for instance i.  
	    		  vecinosTemp[i][j] = obtenerCercano(vecinosTemp[i],j,dMatrix, i, datos, real, nominal, nulo, distanceEu);//return the second closest
	    	  }
	    	  else {
	  ...
	  if (Math.round(ganancia) >= (double)umbral) { //psxhll modify: >= to >
	      for (i=0; i<cuerpo.length; i++) {
	    	  for (j=0; j<K; j++) {
